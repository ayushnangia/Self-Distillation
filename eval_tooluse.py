"""Evaluate SDFT checkpoint on tool-use task via regex matching.

Follows the paper's methodology:
- Greedy decoding (temperature=0) for accuracy
- pass@k with temperature=1.0, top-p=0.95
- Regex matching against ground-truth API call
- Accounts for variations in argument ordering
"""
...
def pass_at_k(n: int, c: int, k: int) -> float:
    """Unbiased estimator of pass@k. n=total samples, c=correct samples, k=k."""
    if n - c < k:
        return 1.0
...
def main():
    ...
    parser.add_argument("--eval_data", type=str, default="data/tooluse_data/eval_data.json")
    ...
    parser.add_argument("--skip_pass_at_k", action="store_true",
                        help="Skip pass@k evaluation (faster)")
    ...
    # 2. pass@k
    pass_at_k_scores = {}
    if not args.skip_pass_at_k:
        print(f"\n>>> pass@k (n={args.n_samples}, k={k_values}, temp=1.0, top_p=0.95) <<<")
        pass_at_k_scores, per_example_pak = eval_pass_at_k(
            llm, tokenizer, eval_data, n_samples=args.n_samples, k_values=k_values
        )
    ...
    output_file = args.output_file or "eval_results_tooluse.json"
